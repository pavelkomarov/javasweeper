...
public class MinesweeperUtils {
	 ...
	 *         array
	 */
	public static <T extends MineTile> ArrayList<T> getAdjacentTiles(int x, int y, T[][] board) {
	...
	public static <T extends MineTile> ArrayList<T> expandTile(T tile, T[][] board) {
		...
		while (!stack.empty()) {
			...
			visited.add(current); // keep track of visited tiles (we don't want
									// an infinite loop!)
			toOpen.add(current);
			if (current.getAdjacentMines((Tile[][])board) == 0) {
				for (T b : getAdjacentTiles(current.getCol(), current.getRow(), board)) {
